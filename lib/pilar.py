...
class Pilar:
    ...
    def __init__(self, screen_width, screen_height, x):
        ...
        gap_height_percent = 0.10 + random() * 0.10
        ...
        self.lower_rect = pygame.Rect(x, screen_height - self.lower_pilar_height, self.gap_width,
                                      self.lower_pilar_height)

    def boundaries(self):
        return {
            "x-boundaries": [self.upper_rect.x, self.upper_rect.x + self.upper_rect.w],
            "y-boundaries": [self.upper_rect.y + self.upper_rect.h, self.lower_rect.y]
        }
